...
for label, url in pages:
    ...
    qr.add_data(url)
    qr.make(fit=True)
    img = qr.make_image(fill_color="#c73a6a", back_color="#fff5f8").convert("RGB")
...
home.add_data(base + "/v1-cute.html")
home.make(fit=True)
home.make_image(fill_color="#c73a6a", back_color="#fff5f8").convert("RGB").save(os.path.join(out_dir, "qr-粉萌招新版.png"))

cell = 360
sheet = Image.new("RGB", (cell * 2, cell * 2 + 80), "#fff5f8")
...
for i, (label, img) in enumerate(files):
    ...
    tw = bbox[2] - bbox[0]
    draw.text((x + (cell - tw) // 2, y + cell - 50), label, fill="#c73a6a", font=font)

title = "科技创新工作室 · 招新扫码方案"
bbox = draw.textbbox((0, 0), title, font=font)
tw = bbox[2] - bbox[0]
draw.text(((cell * 2 - tw) // 2, 25), title, fill="#c73a6a", font=font)

hint = "微信扫码进入首页，点导航可跳转各页面"
bbox = draw.textbbox((0, 0), hint, font=small)
tw = bbox[2] - bbox[0]
draw.text(((cell * 2 - tw) // 2, 56), hint, fill="#8a6e78", font=small)

sheet.save(os.path.join(out_dir, "qr-preview.png"))
print("Generated:", ", ".join([f"qr-{l}.png" for l, _ in pages]) + ", qr-粉萌招新版.png, qr-preview.png")
